<script>
    ...
    const filter = () => {
        ...
        if (minBalance) {
            invoices = invoices.filter(i=>((i.value + i.tax - i.discount) - i.receipt_amount) >= minBalance)
        }
        if (maxBalance) {
            invoices = invoices.filter(i=>((i.value + i.tax - i.discount) - i.receipt_amount) <= maxBalance)
        }
        if (customer_id) {
            invoices = invoices.filter(i=>i.invoice.customer_id===customer_id)
        }
        if (invoiceNum) {
...
{#if receivingPayment}
    <Receipt {currentInvoice} on:close={()=>{receivingPayment = false}}/>
{/if}
...
<section class="flex flex-col">
    ...
    <div class="filters">
        ...
        <div class="group single">
            <label for="vendor">Filter by Customers</label>
            <Select
            class='select'
            id='vendor'
            items={customers}
            ...
            on:change={(e)=>{customer_id=e.detail.id; filter()}}
            on:clear={()=>{customer_id=""; filter()}}
            />
        </div>
        <div class="group single">
    ...
    <div class="invoices">
        ...
        <div class="rows">
            {#each invoices as inv, i (i+1)}
                {#if (inv.value - inv.discount + inv.tax - inv.invoice.rounded_off)-inv.receipt_amount > 0}
                    <div class="row">
                        <span>{i+1}</span>
                        ...
                        <span>{inv.receipt_amount.toFixed(2)}</span>
                        <span>{((inv.value - inv.discount + inv.tax)-inv.receipt_amount-inv.invoice.rounded_off).toFixed(2)}</span>
                        <button class="flex-cc" on:click={()=>{currentInvoice=inv; receivingPayment=true}}>Rec.</button>
                    </div>
                {/if}
            {/each}
        </div>
